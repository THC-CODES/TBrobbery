...
RegisterNetEvent('esx_holdup:robbingTb')
AddEventHandler('esx_holdup:robbingTb', function(currentStore)
    holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_holdup:endBlip')
AddEventHandler('esx_holdup:endBlip', function()
...
AddEventHandler('esx_holdup:robberyComplete', function(award)
    holdingUp, store = false, ''
    ESX.ShowNotification(_U('robbery_finished', award))
end)
                    ...
                    if distance < 0.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if IsPedArmed(PlayerPedId(), 4) then
                                TriggerServerEvent('esx_holdup:robberyBegun', k)
                            else
