...
AddEventHandler('esx_holdup:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_holdup:endBlip', xPlayers[i])
		end
	end
...
RegisterServerEvent('esx_holdup:robberyBegun')
AddEventHandler('esx_holdup:robberyBegun', function(currentStore)
	local _source  = source
	...
	if Stores[currentStore] then
		...
		if not rob then
			if cops >= Config.PoliceNumberRequired then
				...
				TriggerClientEvent('esx_holdup:robbingTb', _source, currentStore)
